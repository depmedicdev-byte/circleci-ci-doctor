...
const { auditConfig, auditDirectory, summarize } = require('../src/index');
...
const HELP = `circleci-ci-doctor - audit .circleci/config.yml for waste, cost, security gaps.

Usage:
  circleci-ci-doctor [path]                 audit .circleci/config.yml in [path] (default: cwd)
  circleci-ci-doctor --file path/to/config.yml
  circleci-ci-doctor --json                 machine-readable output
  circleci-ci-doctor --markdown             markdown table (PR comment friendly)
  circleci-ci-doctor --severity=warn        only warn + error
  circleci-ci-doctor --only=rule-a,rule-b   only run named rules
  circleci-ci-doctor --disable=rule-x       skip these rules
  circleci-ci-doctor --rules                list rules and exit
  circleci-ci-doctor --demo                 audit the bundled bad config (smoke test)
  circleci-ci-doctor --version
  circleci-ci-doctor --help
...
function main() {
  ...
  if (args.version) { process.stdout.write(require('../package.json').version + '\n'); return 0; }
  if (args.listRules) {
  ...
  try {
    if (args.demo) {
      const demo = path.join(__dirname, '..', 'examples', 'bad-circle-config.yml');
      const src = fs.readFileSync(demo, 'utf8');
      findings = auditConfig(src, 'examples/bad-circle-config.yml', args);
    } else if (args.file) {
      const src = fs.readFileSync(args.file, 'utf8');
      findings = auditConfig(src, path.relative(process.cwd(), args.file).replace(/\\/g, '/'), args);
    } else {
      const target = args.positional[0] || process.cwd();
      const stat = fs.existsSync(target) ? fs.statSync(target) : null;
      if (stat && stat.isFile()) {
        const src = fs.readFileSync(target, 'utf8');
        findings = auditConfig(src, path.relative(process.cwd(), target).replace(/\\/g, '/'), args);
      } else {
